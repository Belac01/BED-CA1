        ...
        if (userError) {
            ...
            model.getGuildByUserId(userId, (guildError, guildResult) => {
                if (guildError) {
                    ...
                    res.status(409).send("Error: User is already a member of a guild"); 
                } else {
                    ...
                    model.getGuildByLeaderId(userId, (error, result) => {
                        if (error) {
                            ...
                            res.status(409).send("Error: User already has a guild"); 
                        } else {
                            const data = {
                            ...
                            model.getGuildByName(data.guild_name, (guildError, guildResult) => {
                                if (guildError) {
                                    ...
                                    res.status(409).send("Error: Guild name already exists"); 
                                } else {
    ...
    const callback = (error, results, fields) => {
        if (error) {
            ...
            if (results && results.message) {
                ...
                res.status(404).json({
                    message: "Guild member not found or not currently in a guild"
                });
            } else {
    ...
    const callback = (error, results, fields) => {
        if (error) {
            ...
            if (results && results.affectedRows === 0) {
                ...
                res.status(404).json({
                    message: "Guild not found or User not found or you are not the leader",
                });
                ...
                res.status(500).json({
                    message: "An error occurred while disbanding the guild",
                });
            }
        }
    };
